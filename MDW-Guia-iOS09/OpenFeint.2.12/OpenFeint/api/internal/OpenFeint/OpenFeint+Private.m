...
+ (BOOL)tryCreateDashboardWithModal:(UIViewController*)modal
{
	...
	{
		instance->mOFRootController = [[[OFControllerLoaderObjC loader] load:@"Root"] retain]; // load(@"Root") retain];
	}
...
+ (void)presentRootControllerWithTabbedDashboard:(NSString*)controllerName pushControllers:(NSArray*)pushControllers
{
	...
	{
		tabController = (OFTabbedDashboardController*)[[OFControllerLoaderObjC loader] load:@"TabbedDashboard"];// load(@"TabbedDashboard");
	}
...
+ (NSString*)gameSpecificDeviceIdentifier
{
    ...
    if (!gsdi)
    {
        NSString* prehashed = [NSString stringWithFormat:@"OFGSDI.%@.%@", [OpenFeint clientApplicationId], [UIDevice currentDevice].uniqueIdentifier];
...
+ (OFRequestHandle*)getImageFromUrl:(NSString *)url forModule:(id)module onSuccess:(OFInvocation*)success onFailure:(OFInvocation*)failure
{
    ...
    [request performSelector:@selector(startAsynchronous) withObject:nil afterDelay:0.0f];
