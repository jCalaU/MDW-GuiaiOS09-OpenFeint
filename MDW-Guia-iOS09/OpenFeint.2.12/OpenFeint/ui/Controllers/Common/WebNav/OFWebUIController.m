...
@synthesize webView, navBar, navBarBackground, loadingView, transitionImage, destinationImage;
...
- (void)viewDidLoad {
    ...
    // Destination Image
    self.destinationImage = [[[UIImageView alloc] initWithFrame:contentFrame] autorelease];
    destinationImage.hidden = YES;
    [self.view addSubview:destinationImage];
...
- (void)_animateTransition:(BOOL)isPush {
    ...
    if (isPush) {
        // Animates the web view sliding left to reveal a faded out web view
        // When loaded the view will fade back in
...
- (void)actionContentLoaded:(NSDictionary*)options {
    // Fade in
    if ([webView isHidden] || ( 0 == floor(webView.alpha) ) || ![loadingView isHidden] || ![transitionImage isHidden] || ![destinationImage isHidden] ) {
        CATransition *transition = [CATransition animation];
